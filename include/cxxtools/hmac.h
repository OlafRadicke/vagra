...
{ 
	...
	if(key.length() > key_hash.blockSize())
		_key = key_hash.getDigest();
	_key.resize(key_hash.blockSize());
...
{ 
	Algo key_hash(key);
	std::string o_key_pad(key_hash.blockSize(), 0x5c);
	std::string i_key_pad(key_hash.blockSize(), 0x36);
